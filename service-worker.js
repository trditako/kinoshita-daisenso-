...
if(!window.__chapterRewardsPatchInstalled){
  ...
  for(let i=0;i<chapterRewardCards.length;i++){
    const c=chapterRewardCards[i];
    if(typeof cards!=='undefined' && !cards[chapterRewardIds[i]]) cards.push(c);
  }
  ...
  function chapterRewardSync(){
    if(typeof chapterClears==='undefined'||typeof ownedFlags==='undefined')return;
    let changed=false;
